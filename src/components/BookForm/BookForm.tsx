...
export interface IModifyBookProps {
  book?: IBook
}

const BookForm = (props: IModifyBookProps) => {
  const {book} = props;
  const {register, handleSubmit, setValue, formState: {errors}} = useForm<IBookAdd>();
  if (book) {
    // @ts-ignore
    Object.keys(book).filter(key => key !== 'picture').forEach(key => setValue(key, book[key]))
  }
      ...
      <Grid container direction='column' spacing={2} justifyContent="center" padding='1rem'>
        <form className='add-form' onSubmit={handleSubmit(onSubmit)}>
          <Grid item paddingBottom={2}>
            <Button onClick={goBack} variant="text" id="button" type='button'
                    startIcon={<KeyboardBackspace/>}>Back</Button>
          </Grid>
          <Grid item xs>
          ...
          <Grid container spacing={1} padding='1rem' justifyContent='start'>
            ...
            { !book &&
              ...
              <Button variant="contained" size='large' component="label" endIcon={<UploadIcon/>}>
                <input required={!book} onChange={fileSelectedHandler} type="file" id="book_pic"
                       accept=".jpg, .jpeg, .png"
                />
              </Button>
            </Grid>}
